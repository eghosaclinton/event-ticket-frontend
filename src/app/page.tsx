  ...
  return (
    <div className=' min-w-full text-white font-semibold'>
        <div className="absolute text-left bottom-0 min-w-[50%] max-w-[50%] p-4">
          <h2 className='text-[2.5rem] font-bold'>Get your tickets for the {festivalName}</h2>
          <button className='bg-white text-[#04092C] text-[1rem] px-6 py-3 font-bold rounded-full text-sm'>Book now</button>
        </div>
...
function BrowseItem({category, categoryImg}: BrowseProps){
  return (
    <div className='relative flex flex-col items-center'>
      <div className='outline-4 outline outline-white flex justify-center font-semibold absolute -bottom-4 w-3/5 bg-[#04092C] rounded-full p-2 text-white'>{category}</div>
...
export default function Home() {
  ...
  const BrowseArr = browseData.map(element=>{
    ...
    return <BrowseItem key={index} category={category} categoryImg={categoryImg}/>
  })
    ...
    <div className="w-4/5 my-auto mx-auto flex flex-col gap-8">
          ...
          <section>
            <div className="heading flex justify-between items-center border-b-[1.5px] border-[#D9D9D9] p-4">
              ...
              <button className='text-white bg-[#2C9CF0] font-medium px-4 py-2 rounded-full'>See more</button>
            </div>
          ...
          <section>
          <div className="heading flex justify-between items-center border-b-[1.5px] border-[#D9D9D9] p-4">
              <h3 className='text-2xl font-bold'>Top Picks Near You</h3>
              <button 
                className='flex items-center gap-2 font-medium px-4 py-2 rounded-full'
              >
